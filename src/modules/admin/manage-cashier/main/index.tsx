...
export const useManageCashier = create<States & Actions>((set) => ({
  openAddData: false,
...
export const Main: FC = (): ReactElement => {
  ...
  const { openAddData, openUpdateData, setOpenAddData, setOpenUpdateData } = useManageCashier(); // ZUSTAND
  const [selectedData, setSelectedData] = useState<ICashierAccount>({ id: "", name: "", username: "", role: "", password: "" });
  ...
  const mutationDelete = useMutation({
    mutationFn: (id: string) => DELETECashierAccount(id),
    onSuccess: async () => {
  ...
  const mutationMultipleDelete = useMutation({
    mutationFn: (ids: string[]) => DELETEMultipleCashierAccount(ids),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["GETCashierAccount"] });
      setCheckbox([]);
      setCheckboxCount(0);
    ...
    <main className="px-5">
      <section>
        <div className="flex items-center gap-3 py-5">
          <h2 className="hidden whitespace-nowrap text-xl font-semibold md:block">Cashier Account List</h2>
